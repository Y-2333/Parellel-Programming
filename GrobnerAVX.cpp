...
alignas(32) int gRows[maxrow][maxsize];
alignas(32) int gBasis[numBasis][maxsize];
...
void reset() {
	...
	RowFile.close();
	BasisFile.close();
	RowFile.open("D:\\Groebner\\测试样例9 矩阵列数37960，非零消元子29304，被消元行14921\\被消元行.txt", ios::in | ios::out);
	BasisFile.open("D:\\Groebner\\测试样例9 矩阵列数37960，非零消元子29304，被消元行14921\\消元子.txt", ios::in | ios::out);
	basisMap.clear();
...
int main() {
	...
	readBasis();
	cout << "Timing Special Serial: " << measureFunction(AVX, numExperiments) << " ms" << endl;
	//writeResult(out);
	reset();
}
